...
const CLIENT_LANDING_PAGE_URL = process.env.NODE_ENV ==='production'?"/user/home/":"http://localhost:3000/user/home/";
...
let googelAuthorizationUri = oAuth2Client.generateAuthUrl({
  access_type: "offline",
...
let present=moment();
    ...
    let dateQuery="(modifiedTime > '"+present.utc().format()+"' or createdTime > '"+present.utc().format()+"')";
    let query="mimeType != 'application/vnd.google-apps.folder' and trashed = false and "+dateQuery;
...
app.get("/user/data/read",authCheck, async (req, res) => {
  try {
    ...
    if(typeof session["lastDriveWriteIndex"] === 'undefined')
    {
      ...
      let userDriveInfo=await User.findOne({ email: req.session.user.email},{lastDriveWriteIndx:1,"drives.id":1,"drives.storageType":1,"drives.token":1,"drives.refreshToken":1,"drives.storageLimit":1,"drives.storageUsage":1,"drives.storageUsagePct":1,"drives.folders":1});
      let nextDriveWriteIndex;
      if(userDriveInfo.lastDriveWriteIndx===null || userDriveInfo.lastDriveWriteIndx==="null")
      {
        nextDriveWriteIndex=0;  
      }
      nextDriveWriteIndex=((userDriveInfo.lastDriveWriteIndx + 1) % userDriveInfo.drives.length);
    ...
    User.findOne({email:req.session.user.email},'drives')
    .then((driveItems)=>{   
...
app.get("/google/callback",authCheck,async (req, res) => {
  try {
    ...
    if(!userInfo) {
        res.redirect(`${CLIENT_LANDING_PAGE_URL}${req.session.user.username}/?attempt=add&success=false&message=failed%20to%20save%20token`);     

    }
    const drivePresent=await User.findOne({"email":req.session.user.email,"drives.id":data.id},{"drives.id":1});
    if(drivePresent !== null) {    
      res.redirect(`${CLIENT_LANDING_PAGE_URL}${req.session.user.username}/?attempt=add&success=false&message=drive%20already%20added`);     
    }
    else {
    res.redirect(`${CLIENT_LANDING_PAGE_URL}${req.session.user.username}/?attempt=add&success=true&message=drive%20added&newDriveType=${newDrive.storageType}&newDrive=${newDrive.email}`);     
    ...
    userInfo.save(async (err) => {
      if (err) {
        ...
        fetchFiles(req.session.user.email,query,data.id,req.session.socketio);           
...
//dbx callback starts
  app.get("/dropbox/callback",authCheck,async (req,res)=>{
    ...
    try {
      ...
      const userDriveQuota = await dbx.getDriveMeta(accessToken.token.access_token);
      // console.log("userDriveQuota: ",userDriveQuota);
      

      let driveUsgPct=(userDriveQuota.used/userDriveQuota.allocation.allocated)*100;
      driveUsgPct=Math.round((driveUsgPct + Number.EPSILON) * 100) / 100;
      let newDrive = await new Drive({
        id: user.account_id,
        email: user.email,
      ...
      const drivePresent=await User.findOne({"email":req.session.user.email,"drives.id":userDriveQuota.id},{"drives.id":1});
      if(drivePresent !== null) {
        res.redirect(`${CLIENT_LANDING_PAGE_URL}${req.session.user.username}/?attempt=add&success=false&message=drive%20already%20added`);     
      }
      else {
        ...
        userInfo.save(async (err) => {
          if (err) {
            ...
            console.log("successfully saved data to db!");            
            fetchFiles(req.session.user.email,'',userDriveQuota.id,req.session.socketio);               
...
app.get("/ms/callback",authCheck,async (req,res)=>{
  ...
  try {
    ...
    const userDriveQuota = await graph.getDriveMetaInit(accessToken.token.access_token);
...
app.post("/action/upload/multiple",authCheck, async (req, res) => {
  ...
  let file=new Object({name:upFile.name, mimeType:upFile.mimetype,fullFileExtension:fileext,size:upFile.size});
...
app.post("/action/drive/delete",authCheck,async (req,res)=>{
 ...
 User.updateOne({"email":req.session.user.email},{ $pull: { "drives" : { id: req.body.driveIdDelete} } })
 .then((response)=>{
  res.status(200).json({
    success: true,
...
app.post("/action/file/delete",authCheck,async (req,res)=>{
  ...
  let resp=await User.findOne({"email":req.session.user.email,"drives.files.fileId": {$eq: req.body.id}}, {"drives.files.$":1})
  ...
  if(driveStorageType==="googleDrive"){
    let status=await googleDrive.deleteFile(req.session.user.email,driveToDeleteFrom,fileId);
  ...
  {
    let status=await graph.deleteFile(req.session.user.email,driveToDeleteFrom,fileId);
    console.log("status: ",status);    
    filesPulled=true;
        res.status(200).json({
      success: true,
    ...
    let status=await dbx.deleteFile(req.session.user.email,driveToDeleteFrom,fileFolderType,fileName,fileId);
    console.log("status: ",status);    
    filesPulled=true;
    res.status(200).json({
      success: true,
...
const fetchFiles = async (email,query,newDriveId,socketId) => {
  try {
    ...
    if(typeof newDriveId !== 'undefined')
    {
      ...
      driveItems= await User.findOne({"email":email,"drives.id":newDriveId},'drives');
    }
    else
    {
...
let storageLeft=parseInt(driveIdtoWrite.storageLimit)-parseInt(driveIdtoWrite.storageUsage);
...
if(file.size>storageLeft)
{
  ...
  storageLeft=parseInt(driveIdtoWrite.storageLimit)-parseInt(driveIdtoWrite.storageUsage);
...
if(driveIdtoWrite.storageType==="googleDrive"){
  try{
    let status=await googleDrive.uploadFile(driveIdtoWrite,file) +"for: "+file.name+", ";
  return status;  
  }
  catch(error){
...
{
  try{
    let status=await graph.uploadFile(req.session.user.email,driveIdtoWrite,file);
...
{
  try{
    let status=await dbx.uploadFile(req.session.user.email,driveIdtoWrite,file);
    console.log("status received is : ",status);
  return status;  
  }
  catch(error){
