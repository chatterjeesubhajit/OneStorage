...
router.get("/logout",authorize,async (req,res)=>{
  ...
  // console.log(req.session);
  if(typeof req.session.nextDriveWriteIndex !== 'undefined' || req.session.nextDriveWriteIndex !== 'null') {
  await User.updateOne({"email":req.session.user.email}, {"$set":{"lastDriveWriteIndx": req.session.nextDriveWriteIndex}});    
  }
